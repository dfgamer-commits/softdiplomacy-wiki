...
gold = floor(baseGold * playerGoldMultiplier)</pre><p>For a normal completed flight, the source and destination airport owners each receive the full amount if both airports are still active. For a captured flight, the capturing/current owner receives the full amount alone when it reaches the diversion airport.</p>
    ...
    <h1 id="Passenger_Plane_Behavior">Passenger Plane Behavior</h1><p>The server moves the plane through each local-lane segment at 1.2 tiles per tick and publishes motion plans so the client renders smooth movement. A plane is removed after completing its single route. It is also removed if it becomes inactive, cannot be built, reaches the cap before creation, or—after capture—has no active airport owned by or friendly to its new owner.</p><p>If a plane changes owner, it abandons the old destination, selects the nearest active, completed airport friendly to the new owner, recalculates its curved route from its current position, and continues without freezing.</p>
    ...
    <h1 id="Trade_Relationships">Trade Relationships</h1><p>Players must be allowed to trade when the flight is selected. The ordinary game diplomacy rules remain authoritative. Airport selection never creates self-trade flights. A route whose ownership changes through hijacking is redirected under the new owner’s friendly-airport rules.</p>
  ...
  page(
    ...
    <div class="sd-notice"><strong>Naval twin:</strong> this article mirrors every section and statistic category in the OpenFront Warship article. Fighters reuse warship health, range, firing rhythm, shell damage, patrol envelope, repair formula, retreat percentage, and veterancy rules. Their deliberate changes are air-only movement and targets, 20% faster movement, airport repair, pentagon rendering, and the air-economy price ratio.</div>
    ...
    <h2 id="Target_Priority">Target Priority</h2><p>Automatic targeting prioritizes aircraft in this order:</p><ol><li>Enemy Fighter Jets.</li><li>Enemy Attack Helicopters.</li><li>Capturable Passenger Planes.</li></ol><p>Within a category, the nearest valid target is selected. Fighters ignore warships, trade ships, transport ships, trains, infantry, buildings, and missiles.</p>
    <h2 id="Capturing">Capturing</h2><p>A fighter closes on a passenger plane at 2.4 tiles per tick and captures it at Manhattan distance 5 or less. The target must be hostile, outside the 20-tick piracy-protection window, and the fighter owner must have at least one active, completed <a href="/Port">Port</a>. On capture, the plane changes owner, gains a new protection window, contributes to fighter veterancy, and reroutes to the nearest airport friendly to its new owner. With no valid port, the fighter may still identify the plane visually but cannot pirate it.</p>
