...
const availableIndex = args.indexOf('--available-upstream');
const availableUpstreamRevision = availableIndex >= 0 ? args[availableIndex + 1] : auditedRevision;
...
const customPages = [
  page(
    ...
    <p>The scheduled review target is weekly. The <a href="/Update_Status">recorded audit</a> identifies the last source snapshot; it is not a live health check or a promise that no bugs remain.</p>`,
  ),
  page(
    'Air_Units',
    'Air Units',
...
gold = floor(baseGold * playerGoldMultiplier)</pre><p>For a normal completed flight, the source and destination airport owners each receive the full amount if both airports are still active. For a captured flight, the capturing/current owner receives the full amount alone when it reaches the diversion airport. Normal deliveries contribute to each recipient’s trade-income counter; captured deliveries contribute to the recipient’s piracy-income counter. These counters and post-game trade statistics record the payment rather than award additional gold.</p>
    ...
    <h1 id="Passenger_Plane_Behavior">Passenger Plane Behavior</h1><p>The server moves the plane through local-lane segments at a configured 1.2 tiles per tick and publishes motion plans for rendering. The plane is removed after its route. After capture, it selects the nearest active, completed, unmarked airport owned by its new owner, not an allied airport. If the diversion airport is destroyed or changes owner, the plane selects another owned airport. With no valid owned airport it is deleted.</p>
    <h1 id="Capture_Destruction_Mechanics">Capture/Destruction Mechanics</h1><p>A fighter captures a passenger plane at Manhattan distance 5 or less when all piracy gates pass. The fighter owner must have an active, completed port; the plane must be outside its protection window; the owners must be hostile; and the target must not belong to the fighter owner or a friendly player. A plane heading to the fighter owner’s airport or a friendly player’s airport is also protected. The plane changes ownership intact, gives the fighter capture progress toward veterancy, gains a fresh piracy-protection window, and diverts to the nearest valid owned airport.</p><p>A passenger plane may also be destroyed by any ordinary game effect that can damage or invalidate that unit. Fighter logic captures passenger planes rather than shelling them during normal automatic targeting.</p>
    <h1 id="Trade_Relationships">Trade Relationships</h1><p>Players must be allowed to trade when the airport selects a flight. Selection excludes the source owner’s airports. Ordinary flights recheck destination activity and trade permission before movement or delivery, and cancel when both endpoints have the same owner. Normal completion also requires both airports to remain active. Captured cargo follows the separate owned-airport diversion and piracy-payout path.</p>
  ...
  page(
    ...
    <h1 id="Repair">Repair</h1><p>Fighters use the two warship repair systems, replacing ports with owned airports. A doomed side cannot repair its fighters. When the optional Doomsday Clock applies naval health attrition, it also drains fighter health using the same calculation. A jet automatically starts a repair retreat below 75% of its veterancy-adjusted maximum health after the brief manual-move protection period. It first heads to the nearest owned airport, can switch to a substantially closer airport, waits or diverts when repair slots are full, and resumes patrol when fully healed.</p>
    ...
    <h2 id="Patrol">Patrol</h2><ul><li>Patrols varied waypoints inside the 100-tile ordered area when no target is present.</li><li>Re-evaluates automatic target priority every tick, while preserving a valid explicit intercept order.</li><li>Engages hostile military aircraft that enter its 130-tile detection range.</li><li>Completes its current combat patrol leg instead of restarting that leg every tick. A pursuit leg can stop once the target is within firing range.</li><li>Pursues passenger planes for capture only when piracy requirements pass.</li><li>Returns to patrol after combat, capture, invalidation, or completed repair.</li></ul>
    ...
    <h2 id="Target_Priority">Target Priority</h2><p>Current automatic acquisition order:</p><ol><li>Enemy Attack Helicopters.</li><li>Enemy Fighter Jets.</li><li>Capturable Passenger Planes.</li></ol><p>Within a category, the nearest valid target is selected. Like naval targeting, troop carriers take priority over combat units, then trade units. A newly available helicopter can replace an existing automatic fighter or passenger-plane lock. A valid manual intercept remains the player’s chosen target. Fighters ignore warships, trade ships, transport ships, trains, infantry, buildings, and missiles.</p>
    <h2 id="Capturing">Capturing</h2><p>A fighter closes on a passenger plane at 2.4 tiles per tick and captures it at Manhattan distance 5 or less. The target must be hostile, outside the 20-tick piracy-protection window, and the fighter owner must have an active, completed <a href="/Port">Port</a>. A plane heading to the fighter owner’s airport or a friendly player’s airport cannot be hijacked. On capture, the plane changes owner, receives a new protection window, contributes to veterancy, and diverts to an owned airport; see <a href="/Passenger_Plane">Passenger Plane</a>.</p>
  ...
  page(
    ...
    <h1 id="Retreat">Retreat</h1><p>If the destination becomes invalid or the mission is cancelled into retreat, the helicopter flies to the nearest active, completed owned airport. It rechecks that return airport during flight: destruction, capture, a deletion mark, construction, or invalid terrain triggers selection of another valid owned airport. A completed retreat returns <strong>75%</strong> of the carried troops and loses 25%. If no valid airport exists, the aircraft is deleted and the full carried troop count is returned immediately, once only. In every case the mission ends; the helicopter cannot become a reusable ferry.</p>
  ...
  page(
    ...
    `<p>This page distinguishes the OpenFront revision incorporated in the game from the latest revision checked. A wiki refresh does not mean every upstream game change has been merged.</p>
    <h2 id="Latest_audit">Latest audit</h2><div class="sd-spec-grid"><div class="sd-spec"><span>Audit date</span><strong>${auditedAtDisplay}</strong></div><div class="sd-spec"><span>OpenFront incorporated</span><strong>${auditedRevision.slice(0, 10)}</strong></div><div class="sd-spec"><span>OpenFront available</span><strong>${availableUpstreamRevision.slice(0, 10)}</strong></div><div class="sd-spec"><span>OpenFront wiki</span><strong>${auditedWikiRevision.slice(0, 10)}</strong></div><div class="sd-spec"><span>Wiki coverage</span><strong>Every official source page</strong></div></div>
    ${availableUpstreamRevision !== auditedRevision ? '<div class="sd-notice"><strong>Game upgrade pending:</strong> targeted air-unit fixes have been validated, but the larger upstream merge requires review and approval. Official wiki articles describe their upstream source and may cover changes not yet incorporated in this game build.</div>' : ''}
    ...
    <h2 id="Cadence">Cadence</h2><p>The requested review cadence is weekly. This page is a recorded snapshot, not a live monitor. A scheduled check can be delayed or blocked; use the recorded date rather than assuming every cycle completed.</p>`,
  ),
];

await mkdir(pagesRoot, { recursive: true });

const officialPages = (await loadOfficialPages({ revision: auditedWikiRevision, sourcePath })).map((entry) => ({
  ...entry,
  cats: entry.cats || [],
